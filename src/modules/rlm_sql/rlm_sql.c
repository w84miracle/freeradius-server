...
int sql_set_user(rlm_sql_t const *inst, REQUEST *request, char const *username)
{
	 ...
	 */
	fr_pair_delete_by_child_num(&request->packet->vps, vp->da->parent, vp->da->attr, TAG_ANY);
 ...
 */
#define sql_unset_user(_i, _r) fr_pair_delete_by_child_num(&_r->packet->vps, _i->sql_user->parent, _i->sql_user->attr, TAG_ANY)
...
finish:
	talloc_free(head);
	fr_pair_delete_by_child_num(&request->packet->vps, inst->group_da->parent, inst->group_da->attr, TAG_ANY);
