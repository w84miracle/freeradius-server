...
int fr_dhcp_decode(fr_dict_t *dict, RADIUS_PACKET *packet)
{
	...
	if (giaddr == htonl(INADDR_ANY)) {
		 ...
		 */
		vp = fr_pair_find_by_child_num(head, fr_dict_root(dict), 256, TAG_ANY);
		if (vp && vp->vp_integer == 3) {
			/*
			 *	Vendor is "MSFT 98"
			 */
			vp = fr_pair_find_by_child_num(head, fr_dict_root(dict), 63, TAG_ANY);
			if (vp && (strcmp(vp->vp_strvalue, "MSFT 98") == 0)) {
				vp = fr_pair_find_by_child_num(head, fr_dict_root(dict), 262, TAG_ANY);
	 ...
	 */
	maxms = fr_pair_find_by_child_num(packet->vps, fr_dict_root(dict), 57, TAG_ANY);
	mtu = fr_pair_find_by_child_num(packet->vps, fr_dict_root(dict), 26, TAG_ANY);
