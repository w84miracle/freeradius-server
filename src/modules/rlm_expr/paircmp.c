		     ...
		     UNUSED VALUE_PAIR **reply_pairs)
{
	 ...
	 */
	vp = fr_pair_find_by_child_num(check_pairs, fr_dict_root(fr_dict_internal), PW_STRIP_USER_NAME, TAG_ANY);
	if (vp && !vp->vp_integer) return ret;

	/*
	 *	See where to put the stripped user name.
	 */
	vp = fr_pair_find_by_child_num(check_pairs, fr_dict_root(fr_dict_internal), PW_STRIPPED_USER_NAME, TAG_ANY);
	if (!vp) {
		/*
		 *	If "request" is NULL, then the memory will be
		 *	lost!
		 */
		vp = radius_pair_create(request->packet, &req,
					fr_dict_root(fr_dict_internal), PW_STRIPPED_USER_NAME);
		if (!vp) return ret;
		request->username = vp;
	}
...
void pair_builtincompare_add(void *instance)
{
	...
	paircompare_register(fr_dict_attr_child_by_num(fr_dict_root(fr_dict_internal), PW_PREFIX),
			     fr_dict_attr_child_by_num(fr_dict_root(fr_dict_radius), PW_USER_NAME),
			     false, presufcmp, instance);

	paircompare_register(fr_dict_attr_child_by_num(fr_dict_root(fr_dict_internal), PW_SUFFIX),
			     fr_dict_attr_child_by_num(fr_dict_root(fr_dict_radius), PW_USER_NAME),
			     false, presufcmp, instance);

	paircompare_register(fr_dict_attr_child_by_num(fr_dict_root(fr_dict_internal), PW_CONNECT_RATE),
			     fr_dict_attr_child_by_num(fr_dict_root(fr_dict_radius), PW_CONNECT_INFO),
			     false, connectcmp, instance);

	paircompare_register(fr_dict_attr_child_by_num(fr_dict_root(fr_dict_internal), PW_PACKET_TYPE),
			     NULL,
			     true, packetcmp, instance);

	paircompare_register(fr_dict_attr_child_by_num(fr_dict_root(fr_dict_internal), PW_RESPONSE_PACKET_TYPE),
			     NULL,
			     true, responsecmp, instance);

	for (i = 0; generic_attrs[i] != 0; i++) {
		paircompare_register(fr_dict_attr_child_by_num(fr_dict_root(fr_dict_internal),
							       generic_attrs[i]), NULL, true,
							       genericcmp, instance);
	}
}
