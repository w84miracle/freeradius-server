...
static int mod_instantiate(UNUSED rlm_eap_config_t const *config, void *instance, CONF_SECTION *cs)
{
	...
	dv = fr_dict_enum_by_name(fr_dict_attr_child_by_num(fr_dict_root(fr_dict_internal), PW_AUTH_TYPE), "MS-CHAP");
	if (!dv) dv = fr_dict_enum_by_name(fr_dict_attr_child_by_num(fr_dict_root(fr_dict_internal), PW_AUTH_TYPE), "MSCHAP");
	if (!dv) {
