...
eap_type_t eap_name2type(char const *name)
{
	...
	dv = fr_dict_enum_by_name(fr_dict_attr_child_by_num(fr_dict_root(fr_dict_internal), PW_EAP_TYPE), name);
...
char const *eap_type2name(eap_type_t method)
{
	...
	dv = fr_dict_enum_by_da(fr_dict_attr_child_by_num(fr_dict_root(fr_dict_internal), PW_EAP_TYPE), method);
...
eap_packet_raw_t *eap_vp2packet(TALLOC_CTX *ctx, VALUE_PAIR *vps)
{
	...
	while ((i = fr_pair_cursor_next_by_child_num(&cursor, fr_dict_root(fr_dict_radius), PW_EAP_MESSAGE, TAG_ANY))) {
