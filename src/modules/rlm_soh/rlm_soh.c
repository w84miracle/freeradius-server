			...
			REQUEST *request, char const *fmt)
{
	 ...
	 */
	vp[0] = fr_pair_find_by_child_num(request->packet->vps, fr_dict_root(fr_dict_internal),
					  PW_SOH_SUPPORTED, TAG_ANY);
	if (!vp[0])
		return 0;
	...
	if (strncasecmp(fmt, "OS", 2) == 0) {
		/* OS vendor */
		vp[0] = fr_pair_find_by_child_num(request->packet->vps, fr_dict_root(fr_dict_internal),
						  PW_SOH_MS_MACHINE_OS_VENDOR, TAG_ANY);
		vp[1] = fr_pair_find_by_child_num(request->packet->vps, fr_dict_root(fr_dict_internal),
						  PW_SOH_MS_MACHINE_OS_VERSION, TAG_ANY);
		vp[2] = fr_pair_find_by_child_num(request->packet->vps, fr_dict_root(fr_dict_internal),
						  PW_SOH_MS_MACHINE_OS_RELEASE, TAG_ANY);
		vp[3] = fr_pair_find_by_child_num(request->packet->vps, fr_dict_root(fr_dict_internal),
						  PW_SOH_MS_MACHINE_OS_BUILD, TAG_ANY);
		vp[4] = fr_pair_find_by_child_num(request->packet->vps, fr_dict_root(fr_dict_internal),
						  PW_SOH_MS_MACHINE_SP_VERSION, TAG_ANY);
		vp[5] = fr_pair_find_by_child_num(request->packet->vps, fr_dict_root(fr_dict_internal),
						  PW_SOH_MS_MACHINE_SP_RELEASE, TAG_ANY);
...
static rlm_rcode_t CC_HINT(nonnull) mod_post_auth(void *instance, UNUSED void *thread, REQUEST *request)
{
	...
	vp = fr_pair_find_by_child_num(request->packet->vps, fr_dict_root(dict_dhcp), 43, TAG_ANY);
	if (vp) {
		/*
...
static rlm_rcode_t CC_HINT(nonnull) mod_authorize(UNUSED void *instance, UNUSED void *thread, REQUEST *request)
{
	...
	vp = fr_pair_find_by_child_num(request->packet->vps, vendor_microsoft, PW_MS_QUARANTINE_SOH, TAG_ANY);
	if (!vp) {
