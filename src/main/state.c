					    ...
					    RADIUS_PACKET *packet, fr_state_entry_t *old)
{
	 ...
	 *	old state.
	 */
	vp = fr_pair_find_by_child_num(packet->vps, fr_dict_root(fr_dict_radius), PW_STATE, TAG_ANY);
	if (vp) {
...
void fr_state_to_request(fr_state_tree_t *state, REQUEST *request, RADIUS_PACKET *packet)
{
	 ...
	 */
	if (!fr_pair_find_by_child_num(request->packet->vps, fr_dict_root(fr_dict_radius), PW_STATE, TAG_ANY)) {
