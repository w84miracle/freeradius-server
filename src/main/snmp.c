				     ...
				     UNUSED void *snmp_ctx)
{
	...
	out->length = talloc_array_length(out->datum.strvalue) - 1;
	return 0;
}
				 ...
				 UNUSED void *snmp_ctx)
{
	...
	out->length = dict_attr_sizes[PW_TYPE_INTEGER][0];
				    ...
				    NDEBUG_UNUSED fr_snmp_map_t const *map, void *snmp_ctx)
{
	...
	out->datum.ipaddr = client->ipaddr.ipaddr.ip4addr;
...
int fr_snmp_process(REQUEST *request)
{
	...
	while ((vp = fr_pair_cursor_next_by_ancestor(&request_cursor, fr_snmp_root, TAG_ANY))) {
		...
		if (ret < 0) {
			...
			oid_len = dict_print_attr_oid(oid_str + 1, sizeof(oid_str) - 1, fr_snmp_root, tlv_stack[-(ret)]);

			/* Get the last frame in the current stack */
			for (depth = 0; tlv_stack[depth + 1]; depth++);
			len = dict_print_attr_oid(oid_str + 1, sizeof(oid_str) - 1, fr_snmp_root, tlv_stack[depth]);
