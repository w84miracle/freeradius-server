...
static int CC_HINT(nonnull) rad_check_password(REQUEST *request)
{
	...
	while ((auth_type_pair = fr_pair_cursor_next_by_child_num(&cursor, fr_dict_root(fr_dict_internal),
							     PW_AUTH_TYPE, TAG_ANY))) {
		auth_type = auth_type_pair->vp_integer;
		auth_type_count++;

		RDEBUG2("Using 'Auth-Type = %s' for authenticate {...}",
			fr_dict_enum_name_by_da(auth_type_pair->da, auth_type));
...
rlm_rcode_t rad_postauth(REQUEST *request)
{
	...
	if (vp) {
		...
		RDEBUG2("Using Post-Auth-Type %s",
			fr_dict_enum_name_by_da(vp->da, postauth_type));
	}
	...
	if (!autz_retry) {
		...
		if (tmp) {
			...
			RDEBUG2("Using Autz-Type %s",
				fr_dict_enum_name_by_da(tmp->da, autz_type));
			autz_retry = 1;
			goto autz_redo;
		}
	}
	...
	if (result >= 0 &&
		...
		if (tmp) {
			...
			RDEBUG2("Using Session-Type %s",
				fr_dict_enum_name_by_da(tmp->da, session_type));
		}
