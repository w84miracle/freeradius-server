...
VALUE_PAIR *trigger_args_afrom_server(TALLOC_CTX *ctx, char const *server, uint16_t port)
{
	...
	server_da = fr_dict_attr_child_by_num(fr_dict_root(fr_dict_internal), PW_CONNECTION_POOL_SERVER);
	if (!server_da) {
		ERROR("Incomplete dictionary: Missing definition for \"Connection-Pool-Server\"");
		return NULL;
	}

	port_da = fr_dict_attr_child_by_num(fr_dict_root(fr_dict_internal), PW_CONNECTION_POOL_PORT);
	if (!port_da) {
